...
if 'comment_text' in df_master.columns:
    ...
    df_master.loc[is_reddit & (df_master['type'] == 'post'), 'text'] = df_master['title'].fillna('') + ' ' + df_master['body'].fillna('')
    # For comments, use the comment_text
    df_master.loc[is_reddit & (df_master['type'] == 'comment'), 'text'] = df_master['comment_text']
