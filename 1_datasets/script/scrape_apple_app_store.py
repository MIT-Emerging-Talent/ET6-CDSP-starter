...
REVIEWS_LIMIT_PER_APP = 5000  # Total reviews to try and get for one app across all countries.
...
for app_name, app_url_name in APPS_TO_SCRAPE.items():
    ...
    for country in COUNTRIES_TO_SCRAPE:
        ...
        try:
            ...
            print(f"  -- Could not scrape '{country}' for {app_name}. Skipping. Error: {e}")
            continue
    ...
    output_file = os.path.join(OUTPUT_PATH, f'{app_name.lower()}_apple_app_store_raw.csv')
